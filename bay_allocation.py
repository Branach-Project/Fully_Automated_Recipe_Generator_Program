...
class BayAllocation:
    ...
    def run(self, ladder_type, mo_number):
        ...
        while True:
            ...
            if choice == "1":
                self.allocate_ladder(ladder_type, mo_number)
            elif choice == "2":
                bays_to_clear = input("Enter bay numbers to clear (comma-separated): ")
                bays_to_clear = [int(x) - 1 for x in bays_to_clear.split(",")]
                self.clear_bays(bays_to_clear)
            elif choice == "3":
                self.display_bays()
            elif choice == "4":
                validate_it_is_right_pair = input("scan the other ladder to validate it is the right pair: ")
                if validate_it_is_right_pair == mo_number:
