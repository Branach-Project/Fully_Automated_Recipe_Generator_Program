...
def remove_unreachable_holes(coords):
    ...
    Currently, if a hole is within 6cm of a rung, it will not be drilled/ will be removed from the list. 
    ...
    for i in range(len(coords) - 1, -1, -1):
        if ((coords[i][0] % Pitch) < 60 or ((Pitch - 60) < (coords[i][0] % Pitch))) and (coords[i][0] < 0) and (coords[i][0] > -(RungCount-1)*Pitch):

            if coords[i][-1] == 3 or coords[i][-1] == 7:
                print(coords.pop(i))
    print("---------- Unreachable Holes ----------")
    return coords
